...
def test_missing_classes_up_to_six(tmp_path, target_classes):
    ...
    with pytest.warns(UserWarning, match="less than num_classes"):
        X, y = loader[0]
...
def test_empty_data_directory_raises(tmp_path):
    ...
    # Loader init succeeds; error raised lazily when batch is accessed
    loader = DynamicDataLoader(
        data_dir=str(empty),
        ids=["none.png"],
        batch_size=1,
...
def test_index_out_of_range(tmp_path):
    ...
    Image.fromarray(
        (np.random.choice([0, 255], size=(16, 16))).astype(np.uint8)
    ).save(msks / "only.png")
